...
export const generateAccessToken = (user: any, expiresIn: ms.StringValue = config.JWT_ACCESS_EXPIRATION) => {
  return jwt.sign({ id: user.id, email: user.email, verified: user.verified }, config.JWT_SECRET, { expiresIn });
};

export const generateRefreshToken = (user: any) => {
  return jwt.sign({ id: user.id, email: user.email, verified: user.verified }, config.REFRESH_SECRET, { expiresIn: config.JWT_REFRESH_EXPIRATION });
};
